...
SELECT c.customer_id, c.first_name, c.last_name, bsa.artist_name, SUM(ivl.unit_price*ivl.quantity) AS amount_spent
FROM invoice AS i
JOIN customer AS c 
ON c.customer_id = i.customer_id
JOIN invoice_line AS ivl
ON ivl.invoice_id = i.invoice_id
JOIN track AS t 
ON t.track_id = ivl.track_id
JOIN album AS a 
ON a.album_id = t.album_id
